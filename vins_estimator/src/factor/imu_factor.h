...
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
    IMUFactor() = delete;
    IMUFactor(shared_ptr<IntegrationBase> _pre_integration):pre_integration(_pre_integration)
    {
        // ROS_WARN("imu covar dt %lf", pre_integration->sum_dt);
        // cout<<pre_integration->covariance<<endl;
        // double min_abs_coeff = min(abs(_pre_integration->covariance.maxCoeff()), abs(_pre_integration->covariance.minCoeff()));
        // Eigen::Matrix<double, 15, 15> tmp = Eigen::Matrix<double, 15, 15>::Identity();

        // double max_coeff = tmp.maxCoeff();
        // if(max_coeff > 1e5){
        //     sqrt_info = tmp * (1e5 / max_coeff);
        // }
        // else{
        //     sqrt_info = tmp;
        // }

        // double max_abs_coeff = max(abs(tmp.maxCoeff()), abs(tmp.minCoeff()));
        // if(max_abs_coeff > 1e6){
        //     sqrt_info = tmp * (1e6 / max_abs_coeff);
        // }else{
        //     sqrt_info = tmp;
        // }
